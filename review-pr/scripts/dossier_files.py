#!/usr/bin/env python3
"""Emit the `### Files (#n)` section of a review-pr dossier for one base..head range.
...
def git(repo, *args):
    return subprocess.run(['git', '-C', repo, *args], capture_output=True, text=True, errors='replace', check=True).stdout


def parse_u0(text):
    """-U0 hunks -> [(old_start, head_start, head_end | None, old_count)]. head_end None = pure deletion."""
    out = []
    for line in text.splitlines():
        ...
        out.append((old_start, new_start, None if new_count == 0 else new_start + new_count - 1, old_count))
    return out
...
def head_range(u0, old_start, old_count):
    ...
    # A pure insertion `@@ -a,0 +c,d @@` sits *after* old line a, so it belongs to a -U3 hunk
    # whose old span starts at a+1 (a == 0 for an insertion before line 1).
    matched = [h for h in u0 if (lo <= h[0] <= hi) or (h[3] == 0 and lo - 1 <= h[0] <= hi)]
    if not matched:
        return '?'
...
def main():
    ...
    fields = git(a.repo, 'diff', '--name-status', '-z', '-M', rng).split('\0')
    i = 0
    while i < len(fields) and fields[i]:
        ...
        if status[0] in 'RC':
            ...
            i += 3
            rename = f' ({status[0]} `{old}` → `{path}`)'
            paths = [old, path]  # both paths, or git shows the rename as a whole-file add
        else:
            old, path, rename = None, fields[i + 1], ''
            i += 2
            paths = [path]
        numstat = git(a.repo, 'diff', '--numstat', '-M', rng, '--', *paths).split('\t')
        add, dele = numstat[:2]
        if skip_re and skip_re.search(path):
            skipped.append(f'- `{path}` (+{add} −{dele})')
            continue
        out.write(f'#### `{path}` — {status[0]}, +{add} −{dele}{rename}\n\n')
        if (add == '-' and dele == '-') or 'Binary files' in git(a.repo, 'diff', '--stat=1', '-M', rng, '--', *paths) \
                or git(a.repo, 'diff', '--numstat', '-M', rng, '--', *paths).startswith('-'):
            out.write('binary file; no hunks.\n\n')
            continue
        summary = git(a.repo, 'diff', '--summary', '-M', rng, '--', *paths).strip()
        if 'mode change' in summary:
            out.write(f'`{summary.splitlines()[0].strip()}`\n\n')
        if 'test' not in path and not path.endswith('.md'):
            out.write(importers(a.repo, a.head, path) + '\n\n')
        u0 = parse_u0(git(a.repo, 'diff', '-U0', '-M', rng, '--', *paths))
        for old_start, old_count, header, body in parse_u3(git(a.repo, 'diff', '-U3', '-M', rng, '--', *paths)):
